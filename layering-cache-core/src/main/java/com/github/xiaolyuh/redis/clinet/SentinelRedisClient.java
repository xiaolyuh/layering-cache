...
public class SentinelRedisClient implements RedisClient {
    Logger logger = LoggerFactory.getLogger(SentinelRedisClient.class);
    /**
     * 默认key序列化方式
     */
    ...
    public SentinelRedisClient(RedisProperties properties) {
        RedisURI.Builder builder = RedisURI.builder().withSentinelMasterId(properties.sentinelMaster)
                .withTimeout(Duration.ofSeconds(properties.getTimeout()))
                .withDatabase(properties.getDatabase());

        String[] parts = properties.getSentinelNodes().split("\\,");
        for (String part : parts) {
            HostAndPort hostAndPort = HostAndPort.parse(part);
            builder.withSentinel(hostAndPort.getHostText(), hostAndPort.hasPort() ? hostAndPort.getPort() : RedisURI.DEFAULT_SENTINEL_PORT);
        }
